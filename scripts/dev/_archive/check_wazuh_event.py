...
events = list(
    AgentEvent.objects.filter(session_id=SESSION, kind="result").order_by("id")
)
...
for e in events:
    detail = str(e.detail or "")
    if (
        '"execve"' in detail or "syscheck.diff" in detail or "diff" in detail.lower()
    ) and "crontab" in detail.lower():
        print(f"\n=== Event id={e.id} ===")
        print(detail[:2000])
        break
