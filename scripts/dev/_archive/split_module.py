...
CONFIGS = {
    "agent/dashboard/settings_views.py": {
        ...
        "name2mod": {
            ...
            "agents": [
                "settings_agent_save",
                "settings_workflow_save",
                "settings_trigger_save",
                "settings_trigger_toggle",
                "settings_trigger_delete",
                "settings_escalation_save",
            ],
            "baselines": [
                "settings_baseline_subject_save",
                "settings_baseline_subject_toggle",
                "settings_baseline_subject_delete",
                "settings_baseline_window_save",
                "settings_baseline_recompute",
            ],
            "integrations": [
                "settings_model_save",
                "settings_provider_toggle",
                "settings_mcp_save",
                "settings_mcp_delete",
                "settings_connection_save",
                "settings_connection_test",
                "settings_runtime_save",
                "settings_ti_cache_stats",
                "settings_ti_cache_clear",
            ],
        },
    ...
    "agent/dashboard/runner.py": {
        ...
        "name2mod": {
            ...
            "restart": [
                ...
                "_restart_question",
                "_build_restart_context",
                "_copy_investigation_restart_state",
                "_start_agent_thread",
            ],
            "lifecycle": [
                "start_session",
                ...
                "stop_session",
                "is_processing",
                "active_specialist_for_session",
                "is_active",
                "get_ctx",
                "_session_loop",
                "_run_review_investigation",
            ],
        },
    ...
    "agent/views.py": {
        ...
        "name2mod": {
            ...
            "runs": [
                ...
                "CaseQueueTasksView",
                "CaseWorkspaceView",
                "CaseLatestReportView",
            ],
            "webhooks": [
                "_request_secret",
                "_payload_dict",
                "_trigger_metadata",
                "_start_trigger_dispatch",
                "_handle_configured_webhook",
                "ConfiguredWebhookView",
                "TheHiveWebhookView",
            ],
        },
        "drop": ["log"],
...
def split(src_path, cfg):
    ...
    def is_header_node(n):
        return isinstance(n, (ast.Import, ast.ImportFrom)) or (
            ...
            and any(isinstance(t, ast.Name) and t.id == "log" for t in n.targets)
        )
    ...
    for n in tree.body:
        ...
        if current is None:
            raise SystemExit(
                f"orphan before named node at line {n.lineno} in {src_path}"
            )
    ...
    for mod in mod_order:
        ...
        intra_lines = [
            f"from .{t} import {', '.join(sorted(intra[t]))}" for t in sorted(intra)
        ]
        ...
        print(
            f"  {mod}.py: {len((out / f'{mod}.py').read_text(encoding='utf-8').splitlines())} lines"
        )
