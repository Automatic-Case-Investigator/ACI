...
class ReviewTaskModelTest(unittest.TestCase):
    ...
    def test_unparseable_fails_open(self):
        self.assertIsNone(
            _run(review_task_model(_StubModel("garbage"), **self._kwargs()))
        )
        ...
        content = json.dumps(
            {
                "findings": [
                    {
                        "text": "evt-1 reverse shell to 10.0.2.5",
                        "status": "confirmed",
                        "grounded": True,
                        "novel": True,
                    }
        ...
        content = json.dumps(
            {
                "findings": [
                    {
                        "text": "alert says brute force",
                        "status": "restated",
                        "grounded": True,
                        "novel": False,
                        "reason": "paraphrase of the alert",
                    }
                ],
                "decision": "keep_working",
                "feedback": "Substantiate with a retrieved event id.",
            }
        )
        review = _run(review_task_model(_StubModel(content), **self._kwargs()))
        self.assertTrue(review.keep_working)
        ...
        content = json.dumps(
            {
                "findings": [
                    {
                        "text": "evt-1 C2 callback to 10.0.2.5",
                        "status": "confirmed",
                        "grounded": True,
                        "novel": True,
                    }
    ...
    def test_unqueried_clusters_signal_reaches_the_prompt(self):
        ...
        prompt = _build_review_prompt(
            ...
            signals={
                "evidence_queries": 2,
                "unqueried_clusters": ["2022-01-18T13:34:00Z"],
            },
        )
    ...
    def test_unreported_compromise_signal_reaches_the_prompt(self):
        ...
        prompt = _build_review_prompt(
            ...
            signals={
                "evidence_queries": 3,
                "unreported_compromise_artifacts": [
                    "command: [decoded] /dev/tcp/192.168.130.77 [e1]"
                ],
            },
        )
    ...
    def test_review_system_prompt_is_methodology_first(self):
        ...
        self.assertIn(
            "Decide `keep_working` only when the next action is specific",
            _REVIEW_SYSTEM,
        )
...
class TaskReviewStateTest(unittest.TestCase):
    ...
    def test_feedback_without_rejections_reports_verified_count(self):
        ...
        review = TaskReview(
            findings=_split_findings(data), decision="conclude", feedback_text="done"
        )
