...
class VolumeSummaryTest(unittest.TestCase):
    ...
    def test_tail_is_capped(self):
        post = [
            {"time": f"2022-01-18T{13 + i:02d}:00:00Z", "count": 5} for i in range(10)
        ]
            ...
            json.dumps(
                {
                    ...
                    "bursts": [
                        {
                            "start": "2022-01-18T12:15:00Z",
                            "end": "2022-01-18T12:40:00Z",
                            "total": 410000,
                        },
                        {
                            "start": "2022-01-19T09:20:00Z",
                            "end": "2022-01-19T09:45:00Z",
                            "total": 5000,
                        },
                    ],
                }
    ...
    def test_output_is_cp1252_safe(self):
        summarize_result(
            "get_event_volume",
            self._vol([{"time": "2022-01-18T13:15:00Z", "count": 15}]),
        ).encode("cp1252")
...
class ProfileFieldSummaryTest(unittest.TestCase):
    ...
    def test_empty_rare_says_none(self):
        out = summarize_result(
            "profile_field",
            json.dumps({"field": "rule.id", "rare_values": [], "max_doc_count": 10}),
        )
    ...
    def test_top_values_unchanged(self):
        out = summarize_result(
            ...
            json.dumps(
                {
                    "field": "rule.id",
                    "top_values": [{"value": "31101", "count": 940239}],
                }
            ),
        )
...
class KeywordFallbackSummaryTest(unittest.TestCase):
    def test_or_fallback_flagged(self):
        out = summarize_result(
            "search_keyword",
            json.dumps({"total": 1019121, "events": [{"_id": "x"}], "broadened": True}),
        )
    ...
    def test_too_broad_flagged(self):
        out = summarize_result(
            "search_keyword",
            json.dumps({"total": 87855, "events": [{"_id": "y"}], "too_broad": True}),
        )
        self.assertIn("TOO BROAD", out)

    def test_normal_keyword_result_unchanged(self):
        out = summarize_result(
            "search_keyword", json.dumps({"total": 24, "events": [{"_id": "z"}]})
        )
