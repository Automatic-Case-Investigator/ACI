...
class SelectTargetsTest(unittest.TestCase):
    ...
    def test_priority_orders_ip_first_under_batch_cap(self):
        ...
        picked = select_targets(
            arts, covered=set(), remaining_budget=8, max_per_batch=2
        )
    ...
    def test_dedup_within_batch(self):
        ...
        self.assertEqual(
            len(select_targets(arts, covered=set(), remaining_budget=8)), 1
        )

    def test_skips_covered(self):
        arts = [_Art("ip", "1.1.1.1"), _Art("user", "joe")]
        picked = select_targets(
            arts, covered={corr_dedup_key("ip", "1.1.1.1")}, remaining_budget=8
        )
        self.assertEqual([k for k, _, _ in picked], ["user"])

    def test_remaining_budget_zero(self):
        self.assertEqual(
            select_targets([_Art("ip", "x")], covered=set(), remaining_budget=0), []
        )
...
class DeriveWindowTest(unittest.TestCase):
    def test_bounds_from_event_timestamps_with_padding(self):
        ...
        self.assertEqual(
            end, "2025-04-20T07:00:00Z"
        )  # 05:00 (+0.5s) + 2h, floored to sec
...
class SummarizeCorrelationTest(unittest.TestCase):
    ...
    def test_cross_role_appended(self):
        r = self._result(
            cross_role={
                ...
                "neighbors": {
                    "data.srcuser": [
                        {"value": "victim", "count": 8, "event_ids": ["X1"]}
                    ]
                },
            }
        )
    ...
    def test_too_connected_flag(self):
        content, _, _ = summarize_correlation(
            "ip", "10.0.2.5", self._result(too_connected=True)
        )
    ...
    def test_via_provenance_in_content(self):
        content, _, _ = summarize_correlation(
            "user", "root", self._result(), via="ip:10.0.2.5"
        )
