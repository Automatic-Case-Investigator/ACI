...
class SummarizeKillChainTest(unittest.TestCase):
    def test_orders_tactics_by_kill_chain_and_flags_gaps(self):
        ...
        self.assertLess(
            content.index("Persistence"), content.index("Credential Access")
        )
        self.assertEqual(
            observed, ["Initial Access", "Persistence", "Credential Access"]
        )
    ...
    def test_accepts_raw_json_string_and_bad_input(self):
        ...
        c, o, g = summarize_kill_chain(
            json.dumps(
                {
                    "techniques": [
                        {
                            "id": "T1059",
                            "technique": "Command Interp.",
                            "tactics": ["Execution"],
                            "count": 1,
                        }
                    ]
...
class GapLeadSpecsTest(unittest.TestCase):
    ...
    def test_gap_after_confirmed_phase_is_high_priority_forward_trace(self):
        ...
        specs = gap_lead_specs(
            ["Privilege Escalation"], "wazuh-client", observed=["Initial Access"]
        )
        s = specs[0]
    ...
    def test_gap_before_any_confirmed_phase_stays_rule_out(self):
        ...
        specs = gap_lead_specs(
            ["Initial Access"], "wazuh-client", observed=["Execution"]
        )
        s = specs[0]
        self.assertLessEqual(s["priority"], 74)
...
class _FakeTechClient:
    ...
    def post(self, path, json):
        ...
        return httpx.Response(
            200, json=self._payload, request=httpx.Request("POST", f"https://w{path}")
        )
    ...
    def test_parses_techniques_and_tactics(self):
        payload = {
            ...
            "aggregations": {
                ...
                "by_tactic": {
                    "buckets": [{"key": "Credential Access", "doc_count": 30}]
                },
            },
        }
        fake = _FakeTechClient(payload)
        client = self._client(fake)
        out = client.correlate_techniques(
        ...
        self.assertTrue(
            any(c.get("exists", {}).get("field") == "rule.mitre.id" for c in must)
        )
        self.assertTrue(
            any("term" in c and c["term"].get("agent.name") == "kali" for c in must)
        )
