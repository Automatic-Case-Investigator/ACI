...
def _load_build_observation():
    ...
    obs_dir = os.path.join(project_root, "agent", "runtime", "graph", "observation")
    spec = spec_from_file_location(
        "agent.runtime.graph.observation",
        os.path.join(obs_dir, "__init__.py"),
        submodule_search_locations=[obs_dir],
    )
    module = module_from_spec(spec)
    assert spec and spec.loader
    sys.modules["agent.runtime.graph.observation"] = module
...
class _Fake:
    ...
    def __init__(
        ...
        *,
        ...
        self._sample = (
            sample if sample is not None else [{"_id": "ws1"}, {"_id": "ws2"}]
        )
    ...
    def post(self, path, json):
        ...
        if (
            "by_value" in aggs
        ):  # residue-sample follow-up (must_not dominant, terms+top_hits)
        ...
        for field, buckets in self._sel.items():
            aggregations[W._agg_key(field)] = {
                "buckets": buckets,
                "sum_other_doc_count": self._other.get(field, 0),
            }
        return httpx.Response(
            200,
            json={
...
class SelectivityMapTest(unittest.TestCase):
    def test_discriminator_surfaced_and_residue_sample_is_returned(self):
        ...
        disc = next(
            e for e in result["selectivity_map"] if e["role"] == "discriminator"
        )
    ...
    def test_high_cardinality_field_not_chosen_as_discriminator(self):
        ...
        fake = _Fake(
            ...
            sel_fields={
                "data.dstip": [
                    {"key": f"10.0.0.{i}", "doc_count": 100} for i in range(8)
                ],
            },
        )
        result = _client(fake).search(query=_Q, time_range=_TR)
    ...
    def test_discriminator_ranks_above_signature_and_noise(self):
        fake = _Fake(
            ...
            sel_fields={
                ...
                "data.id": [
                    {"key": "404", "doc_count": 9800},
                    {"key": "200", "doc_count": 200},
                ],  # disc
            },
        )
        result = _client(fake).search(query=_Q, time_range=_TR)
    ...
    def test_share_denominator_is_field_total_not_capped_hits(self):
        ...
        fake = _Fake(
            ...
            sel_fields={
                "data.id": [
                    {"key": "403", "doc_count": 30000},
                    {"key": "200", "doc_count": 500},
                ],
            },
            sel_other={"data.id": 200},
        )  # true field total = 30000+500+200 = 30700
        result = _client(fake).search(query=_Q, time_range=_TR)
...
class BuildObservationDiscriminatorTest(unittest.TestCase):
    def test_selectivity_map_surfaces_as_observation_discriminator(self):
        ...
        obs = build_observation(
            [{"name": "search", "raw": result}], objective="scan tail"
        )
        d = obs["discriminator"]
        ...
        self.assertTrue(
            any("inspect and decode" in m for m in obs["recommended_moves"])
        )

    def test_no_discriminator_when_no_selectivity_map(self):
        build_observation = _load_build_observation()
        obs = build_observation(
            [{"name": "search", "raw": {"total": 3, "hits": {"total": {"value": 3}}}}],
            objective="x",
        )
