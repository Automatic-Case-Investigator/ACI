...
class RunnerLoggingTest(unittest.TestCase):
    def test_scenario_anchor_timestamp_is_json_safe(self):
                ...
                "entry_points": [
                    {
                        ...
                        "anchor_timestamp": datetime(
                            2022, 1, 18, 12, 19, 10, tzinfo=timezone.utc
                        ),
        ...
        self.assertEqual(
            runner._question_for(entry, "alert-1"),
    ...
    def test_resolves_closest_imported_hive_alert(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            manifests.mkdir()
            (manifests / "thehive_manifest.run-1.json").write_text(
                json.dumps(
                    {
                        "scenario": "fox",
        ...
        self.assertEqual(
            runner._question_for(spec.entry_points[0], record["id"]),
    ...
    def test_resolves_hive_alert_without_manifest(self):
        ...
        with tempfile.TemporaryDirectory() as tmp, patch.object(
            runner, "_query_alert_records_for_entry", return_value=records
        ):
        ...
        spec = ScenarioSpec.from_dict(
            {
                ...
                "phases": [],
                "entry_points": [
                    {"id": "recon", "anchor_timestamp": "2022-01-18T12:19:10Z"}
                ],
            }
        )
                ...
                "entry_points": [
                    {
                        ...
                        "anchor_event_id": "1700000000.110417",
                    }
                ],
            }
        )
        ...
        spec = ScenarioSpec.from_dict(
            {
                ...
                "phases": [],
                "entry_points": [
                    {"id": "privilege_escalation", "anchor_event_id": "x"}
                ],
            }
        )
    ...
    def test_question_includes_anchor_hint_when_known(self):
        spec = ScenarioSpec.from_dict(
            {"name": "fox", "phases": [], "entry_points": [{"id": "recon"}]}
        )
        q = runner._question_for(
            spec.entry_points[0], "alert-1", "2022-01-18T13:14:31Z"
        )
        self.assertEqual(
            q,
            "Triage and investigate alert alert-1. "
    ...
    def test_run_logs_trial_start_and_done(self):
        ...
        with tempfile.TemporaryDirectory() as tmp, patch.object(
            ...
            runner, "_resolve_entry_alert", return_value={"id": "alert-1"}
        ), patch.object(
            runner, "_capture_tokens", return_value=_Capture()
        ):
            ids = runner.run("fox", "recon", 1, tmp, log=messages.append)
            meta = json.loads(
                (Path(tmp) / "fox" / "recon" / "1" / "meta.json").read_text()
            )
        ...
        self.assertTrue(
            any(
                "done scenario=fox" in m and "tokens=in:10 out:5 calls:2" in m
                for m in messages
            )
        )
    ...
    def test_run_uses_concurrent_trial_pool(self):
        ...
        def fake_wait(session_id, *_args, **_kwargs):
            ...
            release_waits.set()
            return _Run(
                id=session_id, agent_name="orchestrator", result="session answer"
            )

        def fake_children(session_id, _AgentRunClass):
            return [
                _Run(
                    id=f"run-{session_id}", agent_name="investigation", result="report"
                )
            ]
        ...
        with tempfile.TemporaryDirectory() as tmp, patch.object(
            ...
            runner, "_wait_for_session", side_effect=fake_wait
        ), patch.object(
            runner, "_session_children", side_effect=fake_children
        ):
