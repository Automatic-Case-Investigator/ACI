...
def collapse_key(source: dict):
    ...
    src = (
        data.get("srcip") or data.get("src_ip") or source.get("agent", {}).get("ip", "")
    )
...
def selected_alerts(
    dump_path, labels, min_level, collapse, limit, include_anomalies=True
):
...
class TheHive:
    def __init__(self, url, api_key, verify=True):
        if requests is None:
            sys.exit(
                "This operation needs 'requests'. Install with:  pip install requests"
            )
        self.url = url.rstrip("/")
    ...
    def delete_alert(self, alert_id):
        try:
            r = self.s.delete(
                f"{self.url}/api/v1/alert/{alert_id}", verify=self.verify, timeout=30
            )
...
def patch_created_at_cassandra(entries, container, keyspace):
    ...
    pk_col, pk_type, created_col, created_type, err = _cassandra_alert_schema(
        container, keyspace
    )
    if err:
        print(
            f"  ! cassandra introspection failed (container={container}): {err}",
            file=sys.stderr,
        )
        return 0, len(entries)
    if not pk_col or not created_col:
        print(
            f"  ! could not find alert table's id/createdAt columns in keyspace "
            f"'{keyspace}' (pk={pk_col}, created={created_col})",
            file=sys.stderr,
        )
        return 0, len(entries)
    patched, failed = 0, 0
...
def _es_search_by_ref(es_url, source_ref, verify):
    ...
    return (
        (hits[0]["_index"], hits[0]["_id"], hits[0]["_source"])
        if hits
        else (None, None, None)
    )
    ...
    for e in entries:
        try:
            ...
            if not index:
                ...
                continue
            field = next(
                (k for k in source if "created" in k.lower() and "at" in k.lower()),
                None,
            )
            if not field:
                failed += 1
                print(
                    f"  ! no createdAt-like field on ES doc for {e['ref']} "
                    f"(keys sample: {list(source)[:10]})",
                    file=sys.stderr,
                )
                continue
            ...
            if isinstance(existing, (int, float)):
                ...
                new_val = dt.datetime.fromtimestamp(
                    e["epoch"] / 1000, tz=dt.timezone.utc
                ).strftime("%Y-%m-%dT%H:%M:%S.%fZ")
            r = requests.post(
                f"{es_url}/{index}/_update/{doc_id}",
                json={"doc": {field: new_val}},
                verify=verify,
                timeout=30,
            )
            if r.status_code in (200, 201):
                ...
                failed += 1
                print(
                    f"  ! es patch failed for {e['ref']}: {r.status_code} {r.text[:200]}",
                    file=sys.stderr,
                )
...
def cmd_import(args):
    ...
    labels_path = args.labels or os.path.join(
        "preprocessed", f"{args.scenario}_aminer.labels.json"
    )
    ...
    run_id = (
        args.runid
        or dt.datetime.now().strftime("%Y%m%d-%H%M%S") + "-" + uuid.uuid4().hex[:6]
    )
    run_tag = RUN_TAG_PREFIX + run_id

    rows = list(
        selected_alerts(dump, labels, args.min_level, not args.no_collapse, args.limit)
    )
    ...
    if args.dry_run:
        ...
        for a in alerts[:3]:
            print(
                f"   [{a['severity']}] {a['title']}  refs={a['sourceRef']}  obs={len(a['observables'])}"
            )
        ...
        for i, fut in enumerate(as_completed(futs), 1):
            ...
            if status == "created":
                ...
                if info:
                    ...
                    fix_entries.append(
                        {"id": info, "ref": ref, "epoch": date_by_ref[ref]}
                    )
            elif status == "exists":
                exists += 1
                if info:
            ...
            if i % 500 == 0:
                print(
                    f"  ...{i}/{len(alerts)} (created={created} exists={exists} err={errors})"
                )
    ...
    if args.fix_created_at and fix_entries:
        print(
            f"patching createdAt for {len(fix_entries)} alerts "
        ...
        if args.es_url:
            e_ok, e_fail = patch_created_at_es(
                fix_entries, args.es_url.rstrip("/"), verify=not args.insecure
            )
...
def cmd_teardown(args):
    ...
    remaining = hive.ids_by_tag(
        IMPORT_TAG
        if args.all
        else (RUN_TAG_PREFIX + args.runid if args.runid else IMPORT_TAG)
    )
    print(
        "remaining with that tag:",
        len(remaining),
...
def main():
    p = argparse.ArgumentParser(
        description="Import/teardown TheHive 5 alerts from a preprocessed dump."
    )
    ...
    pi.add_argument(
        "--no-collapse", action="store_true", help="do NOT collapse correlation rules"
    )
    ...
    pi.add_argument(
        "--cassandra-container",
        default="thehive-cortex-misp-wazuh-cassandra-1",
    ...
    pi.add_argument(
        "--es-url",
        default=os.environ.get("THEHIVE_ES_URL", ""),
    ...
    pt.add_argument(
        "--all", action="store_true", help=f"delete every alert tagged '{IMPORT_TAG}'"
    )
