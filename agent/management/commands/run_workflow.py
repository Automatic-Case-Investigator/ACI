...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--payload", default="{}", help="JSON event payload (optional)."
        )
    ...
    def handle(self, *args, **options):
        ...
        if get_binding(event_type) is None:
            raise CommandError(
                f"No workflow binding registered for event '{event_type}'."
            )
        ...
        trigger = Trigger(
            event_type=event_type, case_id=options["case_id"], payload=payload
        )
        run = fire(trigger)
        if run is None:
