...
def index(request):
    ...
    sessions = list(
        AgentRun.objects.filter(agent_name="orchestrator").order_by("-created_at")[
            :_SCAN_LIMIT
        ]
    )
    if sessions_q:
        sessions = _search_runs(
            sessions, sessions_q, lambda r: (r.id, r.question, r.case_id)
        )
    sessions_page = Paginator(sessions, _SESSIONS_PER_PAGE).get_page(
        request.GET.get("sp")
    )
    ...
    active = [
        r
        for r in AgentRun.objects.filter(status__in=ACTIVE_STATES).order_by(
            "-updated_at"
        )[:_SCAN_LIMIT]
    ...
    if active_q:
        active = _search_runs(
            active, active_q, lambda r: (r.id, r.agent_name, r.question, r.case_id)
        )
