...
async def _synthesize_analyst_report(
    ...
    prompt = (
        ...
        "finding. This section is where interpretation and scope live — including Initial Access, "
        "which carries TWO independent facts, each with its own evidence and its own confidence: "
        "the VECTOR (the action that first executed attacker-controlled code or granted access — a "
        "request, a login, a delivered file) and the ORIGIN (the source it came from, whether that "
        "source also appears as a later C2/callback address, and whether attribution to an actor "
        "holds). Establishing one does not establish the other: a confirmed payload-bearing request "
        "whose source cannot be tied to an operator is a CONFIRMED vector with an OPEN origin, and "
        "reporting it as unestablished discards a finding you proved. Mark only what is actually "
        "missing — '⚠ Initial access vector not established' when no mechanism is evidenced, "
        "'⚠ Initial access origin not attributed' when the source is absent or cannot be tied to the "
        "actor — and list whichever you marked under Open Gaps. Never emit a marker for something "
        "this same section just confirmed: a warning that contradicts the sentence above it is a "
        "template artifact, not a finding.\n"
