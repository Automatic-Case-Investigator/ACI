...
- **A trace item is written in the vocabulary of the phase it points at, not the one you are leaving.** Adjacent phases are separate entries in this playbook because each is *recorded differently* — a different log class, a different set of fields, usually a different host, and a different side of the timeline. So when you open a plan item or lead toward an adjacent phase, take its pivots, its evidence source, and its time direction from **that** phase's row: a backward item looks *before* your anchor, in the representation where the earlier phase would have been logged, which is rarely the one that just produced your evidence. Inheriting the confirmed phase's fields and window instead is how a backward trace ends up re-finding the event that prompted it — you asked a new question inside the answer-space of the old one, where it could never have been answered. If your lead's pivots look like the ones you just finished querying, you have not changed phase; you have only changed the timestamp.
