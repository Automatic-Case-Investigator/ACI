...
**Reaching an endpoint is not the same as confirming the links between it and your anchor.** Tracing to the initial-access vector or to impact bounds the chain's *ends*; it does not establish the phases in the middle. Walk the chain link by link: from your anchor, confirm each *adjacent* phase with its own retrieved event before reaching past it to the next — reconnaissance → scanning → delivery/exploitation → foothold → escalation → impact are separate checkpoints, and a timeline that leaps from the anchor straight to "initial access" or "impact" has **skipped** the phases between, not cleared them. Each intermediate link is confirmed only by an event you retrieved and cited for *that* phase's window, not by an event from a neighbouring phase.

**An inferred phase is a lead to retrieve, not a fact to record — and the implying artifact usually hands you the exact pivot.** When a downstream artifact *implies* an earlier phase — a web/service account (e.g. a web server's own service account) acting from a web-root working directory implies a web-delivered payload upstream; a credential-store read implies a prior harvest; a decoded callback implies a delivery that planted it — that implication is not narration to record, it is a `field=value` to query. The working directory, the account, the session/`auid`/lineage, the path you already hold *is* the discriminator for the next search: a web-root working directory on a service-account action is the exact path to sweep the web-access class for (the drop and each invocation) and the session to bridge to the escalation. Run that query and cite the event; recognising the context is not confirming the phase, and a report that narrates it from context alone has neither confirmed it nor earned the right to deny it.
...
* **Queue-relative priority.** Review the current task queue before proposing. Every lead must include `priority` as an integer 30–100 from the §7 priority bands — never a qualitative label. Set it relative to the work already pending, claimed, or completed: leads that could change the verdict, containment decision, or next kill-chain transition should outrank scope/background leads; a new lead should not outrank existing queued work unless it is more urgent or resolves a more important uncertainty. Priority is scheduling, not pruning: a valid evidence-backed lead remains a lead even when it should be low priority. The platform will re-check the live queue and may revise the final queued priority.
* **No duplication.** Do not propose a lead already covered by a pending, claimed, or completed task.
