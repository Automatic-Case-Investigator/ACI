...
**`## Investigation Plan`** — A numbered, prioritized list of immediate next steps. **Derive the items from the Incident Response Playbook** (included in your prompt): match the case to its attack phase(s), then turn each matched phase's **Confirm** questions and **Pivots** into concrete plan items. An alert sits in the *middle* of a chain, so trace both directions from each matched phase's `Trace next` — a backward item (how the actor reached this phase / the entry vector) and a forward item (what this phase enabled / its follow-on impact) — for whichever phase(s) the alert matched, unless evidence already retrieved this cycle conclusively establishes that adjacent phase.

**The forward item must move forward in TIME, not only in phase.** A phase-correct forward item bounded to the alert's own minutes answers nothing: the thing a probe, exploit attempt, or recon burst *enabled* happens after it, often quietly, once the noisy part has stopped. So the forward item's window must extend past the alert's anchor to the end of the configured vicinity — the payload sits in the tail, not in the spike. If profiling showed the alert's activity ending at a cessation point, the forward item covers what follows that point.

**Cover every burst you found, not just the loudest.** If your profiling reported multiple distinct activity bursts (different days, or separated by quiet gaps), each burst that could belong to this incident needs its own item. Reporting two bursts in `## Key Evidence` and then planning only around one is how a chain gets missed — and note that the burst containing the alert is the one that matters most, even when another burst looks more dramatic.
...
- Exact pivots (`field=value` pairs). **Pivot on the narrowest artifact you actually found, not on the entity you started from.** Your `## Key Evidence` names concrete artifacts — file paths, URL paths, commands, decoded payloads, rule IDs, hashes; at least one of those belongs in the pivots of any item that traces from it. An item whose only pivots are entity + time (`data.srcip=…`, `agent.name=…`, `data.dstuser=…`) re-searches the same haystack you already searched and will return the same result: if a probe hit `/wp-admin/` and `/wp-content/debug.log`, the forward item pivots on those paths (and the directories they imply, e.g. `/wp-content/uploads/`), not on the source IP again. Only fall back to a bare entity pivot when no narrower discriminator exists — and say so in the item.
...
- **The anchor is the alert's own timestamp.** While triaging you will surface other timestamps — adjacent logins, earlier bursts, activity on other days — and it is easy to let the most *interesting* one become the centre of the plan. It must not. Every item's window is derived from the alert's own anchor unless the item explicitly names a different anchor and says why that chain, rather than the alert's, is the thing to investigate. An item dated to a different day than the alert, with no such statement, is a bug.
- If the task does not already provide a narrower explicit range, derive each time window from the run's configured default vicinity window in **Current Run**: start = anchor timestamp minus the configured hours, end = anchor timestamp plus the configured hours. Do not hardcode 24h unless **Current Run** explicitly says the configured window is ±24h. If a plan item intentionally uses a narrower explicit range, state why that narrower range is justified instead of the configured vicinity window — a narrow window with no stated reason will be widened back to the configured vicinity before the task runs.
- Expected evidence source (SIEM / FIM / SOAR)
- Priority. **Rank by distance from the alert's own chain first, phase type second.** The alert's forward and backward trace items outrank everything else, because they are what this alert is *for*; entities you discovered in passing rank below them however interesting they look. Within a group, order by phase: C2/callback destination > initial access vector > persistence mechanism > context/correlation, mapped onto 90/85/75/60. A plan whose top-priority item investigates an entity the alert did not name — while the alert's own follow-on sits at the bottom — has the ranking inverted, and the investigation will spend its budget before it reaches the alert.
...
Triage completion standard: triage is bounded but must be GROUNDED. You may complete the handoff once you have grounded the alert in retrieved evidence — loaded the alert record and read its raw fields, profiled the discriminating fields, read the specific raw events that confirm or refute the alert's claim, and correlated the key entities the case warrants — OR capably established a confirmed negative (searched where the evidence would appear, with the right discriminator and window, and it is not there). Loading context and checking historical patterns/feedback/baselines is where triage STARTS, not where it stops: consulting context or running a single empty or over-broad query is not grounding and is not sufficient to complete. Once the alert IS grounded, a truncated, noisy, or empty result at that point is itself a "needs investigation" cue, not a reason to keep drilling — preserve the residual uncertainty in the verdict and plan instead of trying to finish every branch inside triage.
